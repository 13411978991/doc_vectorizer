...
export class McpAgentService {
  ...
  async listSessions(input: { sourceId?: string; sourceIds?: string[] } = {}, tenantId = config.DEFAULT_TENANT_ID) {
    return listMcpSessions({
      tenantId,
      limit: 50,
      sourceIds: input.sourceIds ?? (input.sourceId ? [input.sourceId] : undefined)
    });
  }
  ...
  async runUserMessage(input: {
    ...
    // Initialized to undefined and assigned inside both branches of the
    // try block below. The fallback at the end of the function handles the
    // "no tool flow completed" case (e.g. signal aborted mid-flight).
...
/**
 * Citation-collector inner core shared by `collectAnswerCitations`
 * (tool-call path) and `collectCitationSourcesFromObservations`
 * (agent-observation path). Walks each entry's parsed result, dedupes
 * sections by `chunkId`, caps at `cap`, and assigns an `index` from 1.
 *
 * The two callers differ only in *which fields* get added to each
 * citation beyond the section shape — the web-UI path adds
 * `query` + `toolCallId`; the LLM-prompt path compresses `content`.
 * Both shapes are supplied via `extrasFor(section)`.
 */
function collectCitationsFromEntries(
  entries: Array<{
    result: unknown;
    extrasFor: (section: CitationSection) => Partial<AnswerCitation>;
  }>,
  cap = 5,
): AnswerCitation[] {
  const citations: AnswerCitation[] = [];
  const seenChunkIds = new Set<string>();
  for (const { result, extrasFor } of entries) {
    for (const section of extractCitationSections(result)) {
      if (seenChunkIds.has(section.chunkId)) {
        continue;
      }
      seenChunkIds.add(section.chunkId);
      // Spreading `...section` (which contains `content`) + the partial
      // `extrasFor(section)` is safe because both callers either keep
      // the section-provided `content` or override it with a derived
      // preview (which preserves the field's required type). We cast
      // through `AnswerCitation` so TS doesn't complain about the
      // partial merge; the runtime is checked by the vitest suite.
      citations.push({
...
type CitationSection = Omit<AnswerCitation, "index" | "query" | "toolCallId">;

function collectAnswerCitations(toolCalls: McpToolCallRecord[]): AnswerCitation[] {
  return collectCitationsFromEntries(
    toolCalls
      .filter((toolCall) => toolCall.toolName === "sag_search" && toolCall.status === "SUCCEEDED")
      .map((toolCall) => ({
        result: parseToolJsonResult(toolCall.result),
        extrasFor: () => ({
          query: typeof toolCall.arguments.query === "string" ? toolCall.arguments.query : undefined,
...
function collectCitationSourcesFromObservations(
  ...
  return collectCitationsFromEntries(
    observations
      .filter((observation) => observation.toolName === "sag_search" && !observation.error)
      .map((observation) => ({
        result: parseToolJsonResult(observation.result),
        extrasFor: (section: CitationSection) => ({
