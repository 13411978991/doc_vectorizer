...
export class QwenRerankClient implements RerankClient {
  ...
  private async remoteRerank(settings: AiRuntimeSettings, input: {
    ...
    // The configured LLM base URL may not expose a rerank endpoint (e.g.
    // MiniMax/302ai return 404). We wrap the remote call so any failure
    // falls back to the local lexical rerank instead of breaking search.
    try {
      return await this.remoteRerankOnce(settings, input);
    } catch (error) {
      console.warn("[rerank] remote rerank failed, falling back to local:", (error as Error).message);
      return localScoreRerank(input.query, input.candidates, input.topK);
    }
  }

  private async remoteRerankOnce(settings: AiRuntimeSettings, input: {
    query: string;
    candidates: EventRecord[];
    topK: number;
  }): Promise<string[]> {
    const url = buildRerankUrl(config.RERANK_BASE_URL ?? config.LLM_BASE_URL);
