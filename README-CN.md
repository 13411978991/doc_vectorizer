...
### 方式三：完全本地 BGE（不需要 API Key）

不走云端，本地跑 BGE。数据库 schema 固定 1024 维，选模型时认准 `hidden_size=1024`。

```bash
./scripts/download-bge-model.sh   # 默认下到 ./models/bge-large-zh-v1.5
```

然后在 设置 → AI Provider：Embedding 提供方选 **local-bge**，本地模型路径填脚本打印的目录。切换后第一次调用会加载 pipeline（几秒），后续只跑单条推理。

脚本默认下的是 `Xenova/bge-large-zh-v1.5`（int8 量化，~312 MB）—— 中文检索 SOTA，并且输出 1024 维，是 API provider 的直接替身。
